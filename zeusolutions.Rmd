...
```{r, warning=FALSE}
gender_to_currentroles <- select(black_saber_current_employees, gender, role_seniority) 
...
Based on the model, it is evident that `gpa`, `extracurriculars` and `work_experience` have the most significant effect on passing Phase 1, based on their p-values. Hence I can conclude that the most impactful components of the application form that allowd the applicants to proceed to phase 2 of the hiring process are `gpa`, `extracurriculars` and `work_experience`.
...
promoted_employees <- read.csv("data/black-saber-current-employees.csv")  %>% 
    ...
    mutate(number_of_promotions = length(unique(role_seniority))-1)
```
