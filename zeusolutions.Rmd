...
The linear mixed model of the Writing Skill Score shows similar results to the one for the Technical Skill Score with the most influential fixed effect being `gpa`. Unlike the Technical Skill Score, the second most influential fixed effect for the of the Writing Skill Score is `work_experience`. That is, those with relevant work experience have a higher score than those who do not by an average of 4.1. We also find that those who put 'Woman' for their gender only have a higher score than those who put 'Man' by an average of 3.8.
...
pmfd <- promoted_employees %>% 
  ...
  select(-c(financial_q, leadership_for_level, productivity, salary, avg_leadership)) %>% 
  group_by(employee_id) %>% 
  slice(n())

  # tmp <- c("Needs improvement", "Appropriate for level", "Exceeds expectations")
...
gender_to_quarters%>%
  ...
  geom_bar(preserve="single",position="dodge")+
  theme(axis.text.x = element_text(angle=30))+
  labs(title = "Gender Parity in Number of Quarters Worked of Current Employees",fill="Gender")+
...
gender_to_positions%>%
  ...
  theme(axis.text.x = element_text(angle=30))+
  scale_x_discrete(limits = hierarchy) + 
  labs(title = "Gender Parity in Starting Roles of Current Employees",fill="Gender")+
...
Along with the fairness of their promotion practices, the Black Saber board members also want to know that compensation of employees is set by the same standards of merit. Using the same dataset of Black Saber's current employees, we wish to determine which factors contribute to setting employee salaries.
 ...
 In this section, we are interested in the fairness of salary assignment for current employees at Black Saber. We will be looking how `role_seniority`, `leadership_for_level`,`productivity`, and `gender` effeect salary assignment.
...
salary_employees <- black_saber_current_employees %>%
  arrange(employee_id, financial_q) %>% 
...
cur_salary_employees %>%
  ...
  geom_boxplot() + 
  scale_x_discrete(limits = hierarchy) + 
  labs(title = "Gender Parity in Salaries of Current Employees",fill="Gender")+
  xlab("Role") +
  ylab("Salary")
```
